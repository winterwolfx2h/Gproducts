...
public class TaxController {
  ...
  @PostMapping("/addTaxes")
  public ResponseEntity<?> createTaxes(@RequestBody List<Tax> taxes) {
    List<Tax> createdTaxes = new ArrayList<>();
    List<Integer> existingTaxes = new ArrayList<>();

    for (Tax tax : taxes) {
      if (taxService.existsById(tax.getTaxCode())) {
        existingTaxes.add(tax.getTaxCode());
      } else {
        createdTaxes.addAll(taxService.create(Collections.singletonList(tax)));
      }
    }

    Map<String, Object> response = new HashMap<>();
    response.put("createdTaxes", createdTaxes);
    response.put("existingTaxes", existingTaxes);
