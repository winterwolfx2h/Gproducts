...
public class ProductPriceVersionServiceImpl implements ProductPriceVersionService {
  ...
  public ProductPriceVersion update(int productPriceVersionCode, ProductPriceVersion updatedProductPriceVersion) {
    Optional<ProductPriceVersion> existingProductPriceVersionOptional =
        productPriceVersionRepository.findById(productPriceVersionCode);
    if (existingProductPriceVersionOptional.isPresent()) {
      ...
      if (!existingProductPriceVersion.getName().equals(newName)
          && productPriceVersionRepository.existsByName(newName)) {
        throw new ProductPriceVersionAlreadyExistsException(
            "ProductPriceVersion with name '" + newName + "' already exists.");
      }
  ...
  public ProductPriceVersion findById(int productPriceVersionCode) {
    try {
      Optional<ProductPriceVersion> optionalProductPriceVersion =
          productPriceVersionRepository.findById(productPriceVersionCode);
      return optionalProductPriceVersion.orElseThrow(
          () -> new RuntimeException("ProductPriceVersion with ID " + productPriceVersionCode + " not found"));
    } catch (IllegalArgumentException e) {
  ...
  public ProductPriceVersion findByName(String name) {
    try {
      ...
      return optionalProductPriceVersion.orElseThrow(
          () -> new RuntimeException("ProductPriceVersion with ID " + name + " not found"));
    } catch (IllegalArgumentException e) {
      throw new RuntimeException("Invalid argument provided for finding ProductPriceVersion");
    }
  }
